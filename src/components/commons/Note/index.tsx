...
const Note = ({ noteActions }: IProps) => {
  ...
  const note = useMemo(() => { return noteId ? getNote(noteId.toString()) : '' }, [getNote, noteId])
  ...
  return (
    <div className={styles['cmp-note']}>
      ...
      <textarea className={styles['cmp-note__content']} rows={10} value={content} onChange={onChangeContent} placeholder="Add your notes" />
      <Button className={styles['cmp-note__edit-button']} onClick={onSave}>Save</Button>
    </div>
  )
};
